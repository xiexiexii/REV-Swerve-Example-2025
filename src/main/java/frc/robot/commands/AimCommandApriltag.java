...
public class AimCommandApriltag extends Command {
  ...
  // Constructor
  public AimCommandApriltag(DriveSubsystem driveSubsystem, PhotonHelpers photonSubsystem) {
  ...
  public void end(boolean interrupted) {
    ...
    if (Math.abs(m_photonSubsystem.getYaw3D() - m_aimTarget) < VisionConstants.k_aimThreshold) {
  ...
  public boolean isFinished() {
    return
      // Aim (Angle)
      Math.abs(m_photonSubsystem.getYaw3D() - m_aimTarget) < VisionConstants.k_aimThreshold
  ...
  private double photon_aim_PID() {
    ...
    double targetingAngularVelocity = m_aimController.calculate(m_photonSubsystem.getYaw3D() - m_aimTarget);
